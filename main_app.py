...
Window.clearcolor = (0.3, 0.8, 0.8, 0.3)
btn_color = (1, 0.3, 0.4, 1)
...
class InstrScr(Screen):
    def __init__(self, **kwargs):
        ...
        lbl_1 = Label(text='Введите имя:', halign='right')
        self.in_name = TextInput(multiline=False)
        lbl_2 = Label(text='Введите возраст:', halign='right')
        self.in_age = TextInput(text='7', multiline=False)
        self.btn = Button(text='Начать', size_hint=(0.3, 0.2), pos_hint={'center_x': 0.5})
        self.btn.background_color = btn_color
        self.btn.on_press = self.next
        
        line_1 = BoxLayout(size_hint=(0.8, None), height='30sp')
        line_2 = BoxLayout(size_hint=(0.8, None), height='30sp')

        line_1.add_widget(lbl_1)
        line_1.add_widget(self.in_name)
        line_2.add_widget(lbl_2)
        line_2.add_widget(self.in_age)

        outer = BoxLayout(orientation='vertical', padding=8, spacing=8)

        outer.add_widget(instr)
        outer.add_widget(line_1)
        outer.add_widget(line_2)
...
class PulseScr2(Screen):
    def __init__(self, **kwargs):
        ...
        self.lbl_sec = Seconds(15)
        self.lbl_sec.bind(done=self.sec_finished)
        self.lbl_1 = Label(text='Считайте пульс')
        lbl_result1 = Label(text='Результат:', halign='right')
        self.in_result1 = TextInput(text='0', multiline=False)
        line_1.add_widget(lbl_result1)
        line_1.add_widget(self.in_result1)
        line_2 = BoxLayout(size_hint=(0.8, None), height='30sp')
        ...
        line_2.add_widget(lbl_result2)
        line_2.add_widget(self.in_result2)
    ...
    def sec_finished(self, *args):
        if self.lbl_sec.done == True:
            if self.stage == 0:
                self.stage = 1
                self.lbl_1.text = 'Отдыхайте'
                self.lbl_sec.restart(30)
                self.in_result1.set_disabled(False)
            elif self.stage == 1:
                self.stage = 2
                self.lbl_1.text='Считайте пульс'
